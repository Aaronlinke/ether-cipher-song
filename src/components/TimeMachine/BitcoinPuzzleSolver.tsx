...
import { Bitcoin, Key, Play, Pause, Zap, Target, Brain, Sparkles } from 'lucide-react';
...
export function BitcoinPuzzleSolver() {
  ...
  const [searchMode, setSearchMode] = useState<'sequential' | 'random' | 'delta' | 'urzahl'>('delta');
    ...
    <div className="rounded-xl border border-crypto-purple/30 bg-card/80 backdrop-blur-sm p-6 shadow-xl">
      ...
      <div className="mt-4 p-3 rounded-lg bg-yellow-500/10 border border-yellow-500/30">
        <p className="text-xs text-yellow-600">
          ⚠️ <strong>Hinweis:</strong> Die Bitcoin Puzzles sind mathematische Herausforderungen mit 
          astronomisch großen Suchräumen. Selbst mit Delta-Solver und Ur-Zahl-Heuristik ist das Finden 
          eines Schlüssels extrem unwahrscheinlich ohne spezialisierte Hardware (FPGA/ASIC).
        </p>
      </div>
    </div>
  );
